...
Expo app  --Bearer access JWT-->  ALB (HTTPS when domain is set)
                                      |
                                      v
                              ECS Fargate (1 task, public IP, no NAT)
                                      |
              +-----------------------+-----------------------+
              |                       |                       |
              v                       v                       v
...
| Stack | Name | Owns |
| --- | --- | --- |
| Foundation | `AiFishing-Foundation-{stage}` | VPC, RDS, S3, Cognito, secrets |
| API | `AiFishing-Api-{stage}` | ECR, ECS task definition, ALB, alarms |
| Website cert | `AiFishing-WebsiteCert-{stage}` | CloudFront ACM certificate in **us-east-1** |
| Website | `AiFishing-Website-{stage}` | S3 + CloudFront for `onwaterguide.taobowen.com` |
...
- **Locked hostnames** (Route53 zone `taobowen.com`):
  - Website: `https://onwaterguide.taobowen.com`
...
Subsequent deploys are the GitHub Actions `deploy` job: `mvn test` → Docker build → ECR tag git SHA → `cdk deploy AiFishing-Api-$STAGE -c imageTag=$SHA` → `aws ecs wait services-stable` (ALB target health). ECS circuit breaker rolls back a failed task. Do not run a parallel `aws ecs update-service`.
...
3. `POST /api/v1/admin/lakes/validation-catalog` then the script in [`docs/production-data-bootstrap.md`](production-data-bootstrap.md): live `import` + GIS `process` for Head, Rice, Scugog, Simcoe. **Do not** skip import just because a dataset is `AVAILABLE`.
...
ALB idle timeout is **15 minutes** so synchronous Simcoe import/process can finish. Client `--max-time` 1800. If a run still times out or OOMs, raise `memoryMiB` via CDK context; do not add a second ECS task-definition path.
...
- **MVP always-on cost:** ALB + 1 Fargate task + Single-AZ small RDS + S3 + CloudWatch + Cognito + ECR. **No NAT**, Redis, RDS Proxy, Multi-AZ, or extra workers. Public-IP ECS is a cost-conscious MVP choice, not a hardened private-subnet design.
